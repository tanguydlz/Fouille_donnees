...
Nous avons utilisé la fonction bagging du package adabag.  
Nous avons d'abord tenté de faire un bagging avec les paramètres par défaut de la méthode bagging.
Nous obtenons une F mesure de 0.019126 et une AUC de 0.669.
```{r echo=FALSE}
...
#cm_bag1 #F1 = 0.019126
roc.curve(ytest, pred_bag1$class) #AUC = 0.659
```

Nous avons donc essayé de créer des arbres plus profond en spécifiant quelques options. Nous définissons une profondeur maximale de 30 et un nombre d'arbre égal à 100. Nous obtenons alors une F-mesure de 0.0406977 mais un AUC de 0.543. L'AUC désigne la capacité à retrouvé la classe positive. Notre modèle détecte donc moins bien la classe positive que le précédent.
```{r echo=FALSE}
...
#cm_bag3 #F1 = 0.015423
roc.curve(ytest, pred_bag3$class) #AUC = 0.668
#On a légérement une meilleur Fmesure et la même
```
La F-mesure étant de 0.015423 et l'AUC de 0.668, les performances n'ont pas été améliorées.  

Nous allons donc tenter de combiner les deux premiers modèles. Pour cela, nous récupérons les votes de chaque arbres dans les deux modèles et nous attribuons la classe majoritaire. Nous obtenons alors une F-mesure de 0.0286689 se situant entre les 2 modèles et une AUC de 0.620.
```{r echo=FALSE}
#pred_bag1
#pred_bag2
